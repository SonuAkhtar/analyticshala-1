...
const badge = (extra: React.CSSProperties = {}): React.CSSProperties => ({
  ...
  fontSize: "0.7rem", fontWeight: 600, ...extra,
});
...
const S: Record<string, React.CSSProperties> = {
  ...
  badgeCount: badge({ background: "rgba(99,102,241,0.12)", color: "#818cf8", marginLeft: "0.5rem" }),
  ...
  btnDanger:  { background: "rgba(239,68,68,0.12)", color: "#f87171", border: "1px solid rgba(239,68,68,0.25)", borderRadius: 8, padding: "0.4rem 0.6rem", cursor: "pointer", fontSize: "0.82rem" },
  btnSm:      { padding: "0.35rem 0.65rem", fontSize: "0.78rem" },
  seatsCtrl:  { display: "flex", alignItems: "center", gap: "0.25rem" },
  seatsBtn:   { background: "rgba(99,102,241,0.1)", color: "#818cf8", border: "1px solid rgba(99,102,241,0.2)", borderRadius: 6, width: 26, height: 26, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.7rem" },
  ...
  barTrack:   { height: 5, borderRadius: 99, background: "#0f172a", overflow: "hidden", width: 80, marginBottom: 4 },
  /* Drawer */
  backdrop:   { position: "fixed" as const, inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 40 },
  drawer:     { position: "fixed" as const, top: 0, right: 0, bottom: 0, width: "min(480px,100vw)", background: "#0f172a", borderLeft: `1px solid ${T.border}`, zIndex: 50, display: "flex", flexDirection: "column" as const },
  drawerHead: { padding: "1.25rem 1.5rem", borderBottom: `1px solid ${T.border}`, display: "flex", justifyContent: "space-between", alignItems: "flex-start" },
  ...
  tab:        { padding: "0.7rem 1rem", fontSize: "0.85rem", fontWeight: 500, color: T.textM, background: "none", border: "none", borderBottom: "2px solid transparent", cursor: "pointer" },
...
function Toggle({ checked, onChange }: { checked: boolean; onChange: () => void }) {
  return (
    <label style={{ position: "relative", display: "inline-block", width: 38, height: 22, cursor: "pointer" }}>
      <input type="checkbox" style={{ opacity: 0, width: 0, height: 0, position: "absolute" }} checked={checked} onChange={onChange} />
      <span style={{ position: "absolute", inset: 0, borderRadius: 999, background: checked ? T.accent : "#334155", transition: "background 0.2s" }}>
        <span style={{ position: "absolute", top: 3, left: checked ? 19 : 3, width: 16, height: 16, borderRadius: "50%", background: "#fff", transition: "left 0.2s" }} />
      </span>
    </label>
  );
}
...
export default function AdminWorkshops() {
  ...
  const [drawerTab, setDrawerTab]     = useState<"basic" | "seats" | "content">("basic");
  ...
  function handleEdit(w: Workshop) { setEditing({ ...w }); setDrawerTab("basic"); }
  ...
  function toggleMode(mode: string) {
    ...
    upd("eventMode", cur.includes(mode) ? cur.filter((m) => m !== mode) : [...cur, mode]);
  }
  ...
  async function handleSave() {
    ...
    const { error } = await supabase.from("workshops").upsert({
      ...
      desc: editing.desc, date: editing.date, time: editing.time,
      duration: editing.duration, instructor: editing.instructor,
      category: editing.category, category_color: editing.categoryColor,
      image: editing.image, level: editing.level, event_mode: editing.eventMode,
      price: editing.price, original_price: editing.originalPrice,
      total_seats: editing.totalSeats, seats_left: editing.seatsLeft,
      tags: editing.tags, outcomes: editing.outcomes, who_is_it_for: editing.whoIsItFor,
    });
    if (error) { showToast(error.message, "error"); return; }
    setWorkshops((prev) => prev.map((w) => w.id === editing.id ? { ...editing } : w));
  ...
  async function adjustSeats(id: number, delta: number) {
    ...
    if (!w) return;
    const newSeats = Math.max(0, Math.min(w.totalSeats ?? 0, (w.seatsLeft ?? 0) + delta));
    setWorkshops((prev) => prev.map((x) => x.id === id ? { ...x, seatsLeft: newSeats } : x));
    if (isSupabaseConfigured && supabase) {
      const { error } = await supabase.from("workshops").update({ seats_left: newSeats }).eq("id", id);
  ...
  const filtered = workshops.filter((w) => {
    ...
    return w.title?.toLowerCase().includes(q) || w.instructor?.toLowerCase().includes(q) || w.category?.toLowerCase().includes(q);
  });
  ...
  return (
    <div>
        ...
        <table style={S.table}>
          ...
          <tbody>
            {filtered.length === 0 ? (
              ...
              const pct  = w.totalSeats ? ((w.seatsLeft ?? 0) / w.totalSeats) * 100 : 0;
              const barColor = w.seatsLeft === 0 ? "#ef4444" : pct < 30 ? "#f59e0b" : "#22c55e";
              const statusStyle: React.CSSProperties = w.seatsLeft === 0
                ? badge({ background: "rgba(239,68,68,0.12)", color: "#f87171" })
                : (w.seatsLeft ?? 0) <= 5
                ? badge({ background: "rgba(251,146,60,0.12)", color: "#fb923c" })
                : badge({ background: "rgba(34,197,94,0.12)", color: "#4ade80" });

              return (
                <tr key={w.id}>
                  <td style={tdSt}>
                    <div style={{ fontWeight: 600, color: T.text, fontSize: "0.875rem" }}>{w.title}</div>
                    <div style={{ fontSize: "0.73rem", color: T.textD, marginTop: 2 }}>{w.date}</div>
                  </td>
                  ...
                  <td style={tdSt}>
                    ...
                    <span style={{ fontSize: "0.78rem", color: T.textM }}>{w.seatsLeft}/{w.totalSeats} seats</span>
                  </td>
                  <td style={tdSt}>
                    <span style={badge({ background: "rgba(148,163,184,0.1)", color: "#94a3b8" })}>{w.category}</span>
                  </td>
                  <td style={tdSt}>
                    <span style={statusStyle}>
                      {w.seatsLeft === 0 ? "Full" : (w.seatsLeft ?? 0) <= 5 ? "Almost Full" : "Live"}
                    </span>
                  </td>
                  <td style={tdSt}>
                    <Toggle checked={true} onChange={() => { if (!isSupabaseConfigured) showToast("Connect Supabase to edit visibility", "error"); }} />
                  </td>
                  <td style={tdSt}>
                    <div style={{ display: "flex", alignItems: "center", gap: "0.35rem", flexWrap: "wrap" }}>
                      <button style={{ ...S.btnGhost, ...S.btnSm }} onClick={() => handleEdit(w)}>
                        <i className="fas fa-pen" /> Edit
                      </button>
                      <div style={S.seatsCtrl}>
                        <button style={S.seatsBtn} disabled={(w.seatsLeft ?? 0) <= 0} onClick={() => adjustSeats(w.id, -1)} title="Decrease seats">
                          <i className="fas fa-minus" />
                        </button>
                        <button style={S.seatsBtn} disabled={(w.seatsLeft ?? 0) >= (w.totalSeats ?? 0)} onClick={() => adjustSeats(w.id, 1)} title="Increase seats">
                          <i className="fas fa-plus" />
                        </button>
                      </div>
                      <button style={{ ...S.btnDanger, ...S.btnSm }} onClick={() => setShowConfirm(w.id)}>
      ...
      <div style={{ marginTop: "2rem" }}>
        <p style={{ fontSize: "1rem", fontWeight: 600, color: T.text, marginBottom: "1rem" }}>
          Previous Workshops <span style={S.badgeCount}>{workshopData.previous.length}</span>
        </p>
        <div style={S.tableWrap}>
          <table style={S.table}>
            <thead>
            ...
            <tbody>
              {workshopData.previous.map((w, i) => {
                ...
                return (
                  <tr key={w.id}>
                    <td style={{ ...tdSt, fontWeight: 600, color: T.text }}>{w.title}</td>
        ...
        <>
          ...
          <div style={S.drawer}>
            ...
            <div style={S.body}>
              ...
              {drawerTab === "basic" && (
                <div>
                  ...
                  <div style={S.field}>
                    ...
                    <textarea style={S.textarea} rows={3} value={editing.desc ?? ""} onChange={(e) => upd("desc", e.target.value)} />
                  </div>
                  <div style={{ ...S.field, ...S.fieldGrid }}>
                    <div>
                      <label style={S.label}>Date</label>
                      <input style={S.input} value={editing.date ?? ""} placeholder="March 29, Saturday" onChange={(e) => upd("date", e.target.value)} />
                    </div>
                    <div>
                      <label style={S.label}>Time</label>
                      <input style={S.input} value={editing.time ?? ""} placeholder="10:00 AM – 1:00 PM" onChange={(e) => upd("time", e.target.value)} />
                    </div>
                  </div>
                  <div style={{ ...S.field, ...S.fieldGrid }}>
                    <div>
                      <label style={S.label}>Duration</label>
                      <input style={S.input} value={editing.duration ?? ""} placeholder="3 Hours" onChange={(e) => upd("duration", e.target.value)} />
                    </div>
                    <div>
                      <label style={S.label}>Instructor</label>
                      <input style={S.input} value={editing.instructor ?? ""} onChange={(e) => upd("instructor", e.target.value)} />
                    </div>
                  </div>
                  <div style={{ ...S.field, ...S.fieldGrid }}>
                    <div>
                      <label style={S.label}>Category</label>
                      <input style={S.input} value={editing.category ?? ""} onChange={(e) => upd("category", e.target.value)} />
                    </div>
                    <div>
                      <label style={S.label}>Category Color (hex)</label>
                      <input style={S.input} value={editing.categoryColor ?? ""} placeholder="#16a34a" onChange={(e) => upd("categoryColor", e.target.value)} />
                    </div>
                  </div>
                  <div style={S.field}>
                    <label style={S.label}>Image URL</label>
                    <input style={S.input} value={editing.image ?? ""} onChange={(e) => upd("image", e.target.value)} />
                  </div>
                  <div style={S.field}>
                  ...
                  <div style={S.field}>
                    ...
                    <div style={{ display: "flex", gap: "1.25rem", marginTop: "0.35rem" }}>
                      {["Online", "Offline"].map((mode) => (
                        <label key={mode} style={{ display: "flex", alignItems: "center", gap: "0.4rem", cursor: "pointer", fontSize: "0.875rem", color: T.textM }}>
                          <input
                            type="checkbox"
              ...
              {drawerTab === "seats" && (
                <div>
                  <p style={S.sectionLbl}><i className="fas fa-users" /> Seats & Pricing</p>
                  <div style={{ ...S.field, ...S.fieldGrid }}>
                    <div>
                      <label style={S.label}>Price</label>
                      <input style={S.input} value={editing.price ?? ""} placeholder="₹999" onChange={(e) => upd("price", e.target.value)} />
                    </div>
                    <div>
                      <label style={S.label}>Original Price</label>
                      <input style={S.input} value={editing.originalPrice ?? ""} placeholder="₹1,999" onChange={(e) => upd("originalPrice", e.target.value)} />
                    </div>
                  </div>
                  <div style={{ ...S.field, ...S.fieldGrid }}>
                    <div>
                      <label style={S.label}>Total Seats</label>
                      <input style={S.input} type="number" value={editing.totalSeats ?? ""} onChange={(e) => upd("totalSeats", Number(e.target.value))} />
                    </div>
                    <div>
                      <label style={S.label}>Seats Left</label>
                      <input style={S.input} type="number" value={editing.seatsLeft ?? ""} min={0} max={editing.totalSeats} onChange={(e) => upd("seatsLeft", Number(e.target.value))} />
                    </div>
                  </div>
                  <div style={S.field}>
                    ...
                    <input
                      ...
                      onChange={(e) => upd("tags", e.target.value.split(",").map((t) => t.trim()))}
                    />
                  </div>
                </div>
              )}
                ...
                <div>
                  ...
                  <div style={S.field}>
                    ...
                    <textarea style={S.textarea} rows={5}
                      ...
                      onChange={(e) => upd("whoIsItFor", e.target.value.split("\n"))}
                    />
                  </div>
                </div>
              )}
      ...
      {showConfirm !== null && (
        <div style={S.overlay}>
          <div style={S.confirmBox}>
            ...
            <h3 style={{ color: T.text, marginBottom: "0.5rem" }}>Delete Workshop?</h3>
            <p style={{ color: T.textM, fontSize: "0.875rem", marginBottom: "1.5rem" }}>
              Are you sure you want to remove <strong style={{ color: T.text }}>{toDelete?.title}</strong>? This cannot be undone.
            </p>
